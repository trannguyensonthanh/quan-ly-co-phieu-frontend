...
import { formatCurrency, formatNumber, getPriceChangeClass } from '@/utils/format';
...
const MarketSummary = () => {
  ...
  useEffect(() => {
    // Calculate market statistics from mock data
    const stats = mockStocks.reduce((acc, stock) => {
      const volume = stock.volume;
      const value = stock.volume * stock.currentPrice;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{formatNumber(marketStats.totalVolume)}</div>
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-stock-up">{marketStats.increasingStocks}</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-stock-neutral">{marketStats.unchangedStocks}</span>
            </div>
            <div className="flex items-center space-x-2">
              <ArrowDownIcon className="h-4 w-4 text-stock-down" />
              <span className="text-stock-down">{marketStats.decreasingStocks}</span>
            </div>
          </div>
          ...
          <div className="w-full h-2 bg-gray-200 rounded-full mt-2 overflow-hidden">
            <div className="flex h-full">
              <div 
                ...
                style={{ width: `${(marketStats.increasingStocks / mockStocks.length) * 100}%` }}
              ></div>
              <div 
                className="bg-stock-neutral h-full" 
                style={{ width: `${(marketStats.unchangedStocks / mockStocks.length) * 100}%` }}
              ></div>
              <div 
                className="bg-stock-down h-full" 
                style={{ width: `${(marketStats.decreasingStocks / mockStocks.length) * 100}%` }}
              ></div>
            </div>
          </div>
        </CardContent>
